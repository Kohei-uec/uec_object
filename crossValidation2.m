function accuracy = crossValidation2(cv,idxp, idxn,data_pos, data_neg, learn, myClassify)
    %cross validation
    %cv = 5;
    %idx = 1:100;
    accuracy = zeros(cv);
    ...
    for i = 1:cv
        train_pos=data_pos(find(mod(idxp,cv)~=(i-1)),:);
        eval_pos =data_pos(find(mod(idxp,cv)==(i-1)),:);
        train_neg=data_neg(find(mod(idxn,cv)~=(i-1)),:);
        eval_neg =data_neg(find(mod(idxn,cv)==(i-1)),:);
